...
const initials = computed(() =>
  (form.fullName || user.value?.email || '?')
    .split(' ')
    .map(w => w[0])
    .join('')
    .slice(0, 2)
...
<template>
  <div class="max-w-2xl mx-auto space-y-8">
    <div>
      ...
      <p class="text-sm text-muted-foreground mt-1">Your personal information and account settings.</p>
    </div>
    ...
    <div class="bg-card border border-border rounded-2xl p-6 listeo-shadow space-y-5">
        ...
        <div class="min-w-0">
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span class="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-primary/10 text-primary border border-primary/20">
              <BadgeCheck class="w-3 h-3" /> Business owner
            </span>
          </div>
      ...
      <div class="space-y-1.5">
        ...
        <Input :value="user?.email" disabled class="opacity-60 cursor-not-allowed" />
      </div>
    ...
    <div class="border border-destructive/30 rounded-2xl p-6 space-y-3">
      ...
      <p class="text-sm text-muted-foreground">
        Deactivating your account will permanently remove your access. All your services and bookings
        will become unavailable to customers. This action cannot be undone.
      </p>
    ...
    <Dialog v-model:open="showDeactivateDialog">
      <DialogContent class="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            This will permanently deactivate your account and make all your services unavailable.
            This action cannot be undone.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter class="gap-2 sm:gap-0">
          <Button variant="ghost" @click="showDeactivateDialog = false">Cancel</Button>
          <Button variant="destructive" @click="confirmDeactivate" :disabled="deactivating">
