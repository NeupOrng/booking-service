...
if (isAuthenticated.value) navigateTo(user.value?.role === 'business_owner' ? '/business' : '/services')
...
async function handleSubmit() {
  ...
  try {
    ...
    await navigateTo(user.value?.role === 'business_owner' ? '/business' : '/services')
  } catch (e: any) {
    const msg = e?.data?.message
    error.value = Array.isArray(msg) ? msg.join(', ') : (msg || 'Registration failed. Please try again.')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="min-h-screen flex" style="background: hsl(210, 20%, 97%);">
    ...
    <div
      ...
      style="background: linear-gradient(145deg, hsl(224, 45%, 18%) 0%, hsl(189, 68%, 28%) 100%);"
    >
      <div class="absolute inset-0 opacity-10"
        style="background-image: radial-gradient(circle at 70% 40%, white 1px, transparent 1px); background-size: 50px 50px;">
      </div>
    ...
    <div class="flex-1 flex items-center justify-center px-4 py-12">
      <div class="w-full max-w-sm">
        <!-- Mobile logo -->
        <NuxtLink to="/" class="flex items-center justify-center gap-2 mb-8 lg:hidden">
          <div class="w-9 h-9 rounded-lg bg-primary flex items-center justify-center">
            <CalendarDays class="w-5 h-5 text-white" />
          </div>
          <span class="font-bold text-lg text-foreground">Listeo<span class="text-primary">Book</span></span>
        </NuxtLink>
        ...
        <div class="bg-card rounded-2xl border border-border p-8 listeo-shadow">
          <div class="mb-6">
            ...
            <p class="text-sm text-muted-foreground mt-1">Start booking services today</p>
          </div>
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <Input id="fullName" v-model="fullName" type="text" placeholder="Jane Doe" required autocomplete="name" class="h-11" />
            </div>

            <div class="space-y-1.5">
              <label class="text-sm font-medium text-foreground" for="email">Email address</label>
              <Input id="email" v-model="email" type="email" placeholder="you@example.com" required autocomplete="email" class="h-11" />
            </div>

            <div class="space-y-1.5">
              <label class="text-sm font-medium text-foreground" for="password">Password</label>
              <Input id="password" v-model="password" type="password" placeholder="Min. 8 characters" required minlength="8" autocomplete="new-password" class="h-11" />
            </div>

            <Button type="submit" class="w-full h-11 font-semibold" :disabled="loading">
          ...
          <a :href="googleAuthUrl" class="w-full block">
            <Button variant="outline" class="w-full h-11 gap-2 font-medium" type="button">
          ...
          <p class="text-center text-sm text-muted-foreground mt-5">
            ...
            <NuxtLink to="/auth/login" class="text-primary font-semibold hover:underline">Sign in</NuxtLink>
          </p>
        </div>
      </div>
    </div>
