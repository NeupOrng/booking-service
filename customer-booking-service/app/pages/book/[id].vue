...
async function executeBooking() {
    ...
    try {
        ...
        if (status === 409) {
            ...
            notify.error('Booking failed', submitError.value);
        }
    } finally {
    ...
    <div class="container mx-auto px-4 py-8 max-w-3xl">
        ...
        <div v-else-if="store.service">
            ...
            <Card v-if="store.step === 1" class="p-6">
                ...
                <div
                    class="flex gap-2 overflow-x-auto pb-4 scrollbar-hide mb-6"
                >
                    <button
                        v-for="day in days"
                        :key="day.isoDate"
                        ...
                        :class="[
                            'flex flex-col items-center justify-center rounded-xl min-w-[64px] py-2.5 border transition-colors',
                            tempDate === day.isoDate
                                ? 'bg-primary text-white border-primary'
                                : 'bg-background hover:bg-accent border-border',
                        ]"
                    >
                        <span
                            class="text-xs uppercase font-medium opacity-80"
                            >{{ day.dayLabel }}</span
                        >
                ...
                <div
                    ...
                    class="p-6 text-center border rounded-xl bg-muted/30 mb-6"
                >
                    <p class="text-sm">
                ...
                <div v-else class="grid grid-cols-3 gap-2 mb-6">
                    <button
                        ...
                        :class="[
                            'py-2 px-3 text-sm font-medium rounded-xl border transition-colors',
                            !slot.available
            ...
            <Card v-if="store.step === 2" class="p-6">
                ...
                <div class="border rounded-xl divide-y mb-6">
                    ...
                    <div class="flex justify-between p-4 bg-muted/20">
                        ...
                        ><span>{{ store.service.business.name }}</span>
                    </div>
                    <div class="flex justify-between p-4">
                        <span class="text-muted-foreground">Date</span
                        ><span class="font-medium">{{
                            formatBookingDate(store.selectedDate!)
                        }}</span>
                    </div>
                    <div class="flex justify-between p-4">
                        <span class="text-muted-foreground">Time</span
                        ><span class="font-medium">{{
                            formatBookingTime(store.selectedTime!)
                        }}</span>
                    </div>
                    <div class="flex justify-between p-4">
                        <span class="text-muted-foreground">Duration</span
                        ><span>{{ store.service.duration_minutes }} min</span>
                    </div>
                    <div
                        class="flex justify-between p-4 text-lg font-bold bg-muted/30"
                    >
                        <span>Total</span
                ...
                <Alert class="mb-8 border-primary/40 bg-accent">
                    ...
                    <AlertTitle class="text-primary font-semibold"
                        >Cancellation Policy</AlertTitle
                    >
                ...
                <div
                    class="flex justify-between flex-row-reverse border-t pt-6 gap-4"
                >
                    <Button
                        @click="confirmStep2"
                        class="h-11 px-8 w-full sm:w-auto"
            ...
            <Card v-if="store.step === 3" class="p-6 max-w-md mx-auto">
                ...
                <form @submit.prevent="handleLogin" class="flex flex-col gap-4">
                    ...
                    <p
                        class="text-center text-sm text-muted-foreground border-t pt-4"
                    >
                        No account?
                        <NuxtLink
                            :to="`/auth/register?redirect=/book/${serviceId}`"
            ...
            <Card
                v-if="store.step === 4"
                class="p-8 text-center min-h-[400px] flex flex-col items-center justify-center"
            >
                ...
                <template v-else-if="submitError">
                    ...
                    <Button @click="executeBooking" class="h-11 px-8"
                        >Try again</Button
                    >
                </template>
                ...
                <template v-else-if="store.bookingReference">
                    <CheckCircle2
                        class="w-[52px] h-[52px] text-green-500 mb-6"
                    />
                    ...
                    <div
                        class="border rounded-xl divide-y mb-8 text-left max-w-md w-full mx-auto"
                    >
                        <div class="flex justify-between p-4 bg-muted/20">
                        ...
                        <div class="flex justify-between p-4">
                            ...
                            <span class="font-medium"
                                >{{ formatBookingDate(store.selectedDate!) }} at
                                {{
                                    formatBookingTime(store.selectedTime!)
                                }}</span
                            >
                    ...
                    <div
                        class="flex flex-col sm:flex-row gap-3 w-full max-w-md mx-auto"
                    >
                        <Button
                            @click="useRouter().push('/account/bookings')"
                            class="flex-1 h-11"
                            >View my bookings</Button
                        >
                        <Button
                            @click="useRouter().push('/services')"
                            variant="outline"
                            class="flex-1 h-11"
                            >Book another</Button
                        >
