...
import { LayoutDashboard, Calendar, ListChecks, User, Menu, X, LogOut } from 'lucide-vue-next'
...
<template>
  <div class="h-screen flex overflow-hidden bg-background font-sans antialiased">
    ...
    >
      <!-- Logo -->
      <div class="h-14 flex items-center gap-2.5 px-5 border-b border-white/10 shrink-0">
        <div class="w-7 h-7 rounded-lg bg-primary flex items-center justify-center">
          <Calendar class="w-4 h-4 text-white" />
        </div>
        <span class="font-bold text-white text-sm tracking-wide">Listeo<span class="text-primary">Book</span></span>
      </div>
      ...
      <div class="px-4 py-4 border-t border-white/10 shrink-0 space-y-3">
        <div class="flex items-center gap-2.5">
          <div class="w-7 h-7 rounded-full bg-primary/30 flex items-center justify-center shrink-0">
            <span class="text-xs font-bold text-white">{{ user?.fullName?.charAt(0)?.toUpperCase() }}</span>
          </div>
          <div class="min-w-0 flex-1">
            <p class="text-xs font-medium text-white truncate">{{ user?.fullName }}</p>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      <!-- Top bar -->
      <header class="h-14 flex items-center justify-between px-6 border-b border-border bg-card shrink-0">
        <button class="md:hidden text-muted-foreground hover:text-foreground" @click="sidebarOpen = !sidebarOpen">
          <X v-if="sidebarOpen" class="w-5 h-5" />
          <Menu v-else class="w-5 h-5" />
        </button>
        <div class="hidden md:block" />
        <div class="flex items-center gap-3">
          <span class="hidden sm:block text-sm text-muted-foreground">{{ user?.fullName }}</span>
