...
const selectedDayLabel = computed(() => {
    ...
    return day.isToday
        ? 'Today'
        : format(parseISO(props.selectedDate), 'EEE, MMM d');
});

const hasAnyAvailable = computed(() =>
    Object.values(props.groupedSlots)
        .flat()
        .some((s) => s.available),
);
...
watch(
    ...
    async (val) => {
        if (!val) return;
        await nextTick();
        const idx = props.days.findIndex(
            (d) => d.isoDate === props.selectedDate,
        );
        if (idx >= 0 && dayStripRef.value) {
            const chips =
                dayStripRef.value.querySelectorAll<HTMLElement>('[data-chip]');
...
<template>
    <Sheet :open="open" @update:open="emit('update:open', $event)" >
        ...
        >
            <!-- Grabber handle -->
            <div
                class="flex justify-center pt-2.5 pb-0.5 shrink-0"
                aria-hidden="true"
            >
            ...
            <!-- Header -->
            <div
                class="flex items-start justify-between gap-3 px-5 pt-2 pb-3 shrink-0"
            >
                <div>
                    <h3
                        class="text-[18px] font-bold leading-tight text-foreground"
                    >
                        Pick a time
                    </h3>
                    <p
                        v-if="service"
                        class="text-[12px] font-medium text-muted-foreground mt-1"
                    >
            ...
            <div class="px-5 shrink-0">
                ...
                <div class="flex items-center justify-between mb-2">
                    ...
                    <span
                        class="text-[11px] font-semibold text-muted-foreground uppercase tracking-[.04em]"
                    >
                        {{ monthStripLabel }}
                    </span>
                    <button
                ...
                <div
                    ref="dayStripRef"
                    class="flex gap-1.5 overflow-x-auto pb-2 scrollbar-hide"
                >
                    <button
                        v-for="day in days"
                        :key="day.isoDate"
                        ...
                        :style="
                            day.isSoldOut
                                ? 'background:repeating-linear-gradient(135deg,#fff 0 5px,#f3f4f6 5px 10px)'
                                : ''
                        "
                        :class="[
                    ...
                    >
                        <span
                            class="text-[9px] uppercase tracking-[.08em] font-medium opacity-70 leading-none"
                            >{{ day.dayLabel }}</span
                        >
                        <span
                            class="text-[16px] font-bold leading-tight mt-1"
                            >{{ day.dateLabel }}</span
                        >
                    </button>
                </div>
            ...
            <div
                class="flex-1 overflow-y-auto overscroll-contain px-5 py-4 space-y-5 min-h-0"
            >
                <!-- Loading -->
                ...
                <template v-else>
                    ...
                    >
                        ...
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            />
                            <h4
                                class="text-[11px] font-semibold text-muted-foreground uppercase tracking-[.08em]"
                            >
                                {{ period.label }}
                            </h4>
                            <span
            ...
            >
                ...
                <Transition name="slide-up">
                    <div
                        v-if="selectedTime && service"
                        class="flex flex-col min-w-0 flex-1"
                    >
                        <span
                            class="text-[14px] font-bold text-foreground leading-tight"
                        >
...
.custome-sheet-content .custom-sheet-close {
    display: none;
}
</style>
