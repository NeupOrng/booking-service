...
function validate(): boolean {
  ...
  if (isNaN(price) || price <= 0) errors.priceDisplay = 'Price must be greater than 0'
  if (!form.durationMinutes || form.durationMinutes < 5) errors.durationMinutes = 'Duration must be at least 5 minutes'
...
<template>
  <form @submit.prevent="handleSubmit" class="space-y-5">
    ...
    <div class="space-y-1.5">
      <label class="text-sm font-medium">Service Name <span class="text-destructive">*</span></label>
    ...
    <div class="space-y-1.5 flex flex-col">
      ...
      <Textarea v-model="form.description" placeholder="Describe what customers can expect..." rows="3" />
    </div>

    <!-- Price + Duration row -->
    <div class="grid grid-cols-2 gap-4">
      <div class="space-y-1.5">
        <label class="text-sm font-medium">Price (USD) <span class="text-destructive">*</span></label>
        <div class="relative">
          <span class="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-sm">$</span>
          <Input v-model="form.priceDisplay" type="number" step="0.01" min="0.01" class="pl-7" placeholder="45.00" />
        </div>
        <p v-if="errors.priceDisplay" class="text-xs text-destructive">{{ errors.priceDisplay }}</p>
      </div>
      <div class="space-y-1.5">
        <label class="text-sm font-medium">Duration (min) <span class="text-destructive">*</span></label>
        <Input v-model="form.durationMinutes" type="number" min="5" step="5" placeholder="60" />
        <p v-if="errors.durationMinutes" class="text-xs text-destructive">{{ errors.durationMinutes }}</p>
      </div>
    </div>
    ...
    <div class="space-y-1.5 flex flex-col">
      ...
      <Textarea v-model="form.cancellationPolicy" placeholder="e.g. Free cancellation up to 24 hours before..." rows="2" />
    </div>

    <!-- Active toggle -->
    <div class="flex items-center justify-between p-4 bg-muted/40 rounded-xl">
      <div>
        <p class="text-sm font-medium">Active listing</p>
        <p class="text-xs text-muted-foreground">Customers can find and book this service</p>
      </div>
