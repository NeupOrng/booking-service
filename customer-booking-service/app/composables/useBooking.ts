...
export function useBooking() {
    ...
    async function fetchServices(params: {
        ...
        const api = getApi();
        const raw = await api<{ data: BackendServiceSummary[]; meta: any }>(
            '/services',
            {
                query: {
                    ...(params.q ? { q: params.q } : {}),
    ...
    async function fetchMyBookings(query?: {
        ...
        const api = getApi();
        const result = await api<{ data: BookingResponse[]; meta: Meta }>(
            '/bookings/my',
            {
                query: { ...query },
            },
    ...
    async function fetchBookingStats(): Promise<{
        ...
        const [upcomingRes, completedRes] = await Promise.all([
            api<{ meta: Meta }>('/bookings/my', {
                query: { status: 'pending,confirmed', perPage: 1 },
            }),
            api<{ meta: Meta }>('/bookings/my', {
                query: { status: 'completed', perPage: 1 },
            }),
        ]);
        return {
