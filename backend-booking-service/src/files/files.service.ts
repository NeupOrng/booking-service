...
export class FilesService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    await this.minioService.ensureBucket(this.defaultBucket);
  }
  ...
  async uploadFile(
    ...
    const objectKey = `${subfolder ?? 'uploads'}/${Date.now()}-${uuidv4()}-${safeName}`;

    await this.minioService.upload(this.defaultBucket, objectKey, file.buffer, file.mimetype);
  ...
  async getFileUrls(fileIds: string[]): Promise<Record<string, string>> {
    ...
    const entries = await Promise.all(
      records.map(async (file) => {
        const url = await this.minioService.getPresignedUrl(file.bucket, file.objectKey);
  ...
  async deleteFile(fileId: string): Promise<void> {
    const file = await this.filesRepository.findById(fileId);
    if (!file) throw new NotFoundException('File not found');
    await this.minioService.delete(file.bucket, file.objectKey);
