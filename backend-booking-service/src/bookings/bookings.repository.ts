...
export class BookingsRepository {
    ...
    async customerStats(
        customerId: string,
    ): Promise<{ upcoming: number; completed: number; totalSpent: number }> {
        const today = new Date().toISOString().substring(0, 10);

        const [upcomingResult, completedResult, spentResult] =
            await Promise.all([
                this.db.db
                    .select({ count: count() })
                    .from(bookings)
    ...
    async findByBusiness(
        ...
        if (query.dateFrom)
            conditions.push(gte(bookings.bookingDate, query.dateFrom));
        if (query.dateTo)
            conditions.push(lte(bookings.bookingDate, query.dateTo));
