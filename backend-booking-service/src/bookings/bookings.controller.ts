...
export class BookingsController {
  ...
  @ApiResponse({ status: 404, description: 'Service not found / slot not available' })
  ...
  @ApiResponse({ status: 200, schema: { example: { upcoming: 2, completed: 5, totalSpent: 45000 } } })
  @Get('my/stats')
  ...
  @ApiResponse({ status: 403, description: 'Not own booking / already cancelled / completed' })
  @HttpCode(200)
  @Post('my/:id/cancel')
  cancelMyBooking(
    @Param('id') id: string,
  ...
  @ApiOperation({ summary: "List bookings for the current business owner's business" })
  @ApiResponse({ status: 200, description: 'Paginated booking list with customer info' })
  ...
  @ApiOperation({ summary: 'Business owner updates a booking status (confirmed → completed)' })
  @ApiParam({ name: 'id', description: 'Booking UUID' })
