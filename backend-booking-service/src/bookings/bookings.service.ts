...
function toBookingResponse(row: any, includeCustomer = false) {
  ...
  return {
    ...
    ...(includeCustomer ? { customer: { id: customer.id, fullName: customer.fullName, email: customer.email } } : {}),
  };
}
...
export class BookingsService {
  ...
  async createBooking(dto: CreateBookingDto, customerId: string) {
    ...
    if (!slot) throw new NotFoundException('This time slot is not available');
    if (!slot.available) throw new ConflictException('This time slot is fully booked');
  ...
  async listMyBookings(customerId: string, query: CustomerBookingListQueryDto) {
    const page    = query.page    ?? 1;
    const perPage = query.perPage ?? 10;
    const { rows, total } = await this.bookingsRepository.findByCustomer(customerId, query);
    return {
      data: rows.map((r) => toBookingResponse(r, false)),
  ...
  async cancelMyBooking(bookingId: string, customerId: string, dto: CancelBookingDto) {
    const row = await this.bookingsRepository.findById(bookingId);
    if (!row) throw new NotFoundException('Booking not found');
    if (row.booking.customerId !== customerId) throw new ForbiddenException('You do not own this booking');
    if (row.booking.status === 'cancelled') throw new ForbiddenException('This booking is already cancelled');
    if (row.booking.status === 'completed') throw new ForbiddenException('You cannot cancel this booking');
    ...
    return toBookingResponse({ ...row, booking: updated }, false);
  }

  // ── Business owner endpoints ────────────────────────────────────────────────

  async listBusinessBookings(userId: string, userRole: string, query: BusinessBookingListQueryDto) {
  ...
  async updateBookingStatus(bookingId: string, dto: UpdateBookingStatusDto, userId: string, userRole: string) {
    const row = await this.bookingsRepository.findById(bookingId);
  ...
  async cancelBusinessBooking(bookingId: string, userId: string, userRole: string, dto: CancelBookingDto) {
    const row = await this.bookingsRepository.findById(bookingId);
